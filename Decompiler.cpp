    ...
    while (1) {
            ...
            if (_cmp) {
                ...
                if (_cmpRes == CMP_BRANCH) {
                    ...
                    if (_disInfo.Immediate < _curAdr) {
                        _line = "if (" + CmpInfo.L + " " + GetDirectCondition(CmpInfo.O) + " " + CmpInfo.R +
                                ") then Continue;";
                        Env->AddToBody(_line);
                        _curPos += _instrLen;
                        _curAdr += _instrLen;
        ...
        if (_op == OP_TEST || _op == OP_BT) {
            ...
            if (_bytesToSkip) {
                ...
                _line = GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " := Length(" + GetDecompilerRegisterName(
                            DisInfo.OpRegIdx[0]) + ");";
                Env->AddToBody(_line);
        ...
        if (_op == OP_AND) {
            ...
            if (_bytesToSkip) {
                ...
                _line = GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " := " +
                        GetDecompilerRegisterName(DisInfo.OpRegIdx[0]) + " mod " + String(_mod);
                Env->AddToBody(_line);
...
bool __fastcall TDecompiler::SimulateCall(DWORD curAdr, DWORD callAdr, int instrLen, PMethodRec recM, DWORD AClassAdr) {
    ...
    if (IsValidCodeAdr(callAdr)) {
        ...
        if (IsFlagSet(cfImport, Adr2Pos(callAdr))) {
            if (!CheckPrototype(_recN)) {
                ...
                _value = ManualInput(CurProcAdr, curAdr,
                                     "Input return bytes number (in hex) of procedure at " + Val2Str8(curAdr),
                                     "Bytes:");
                if (_value == "") {
                    Env->ErrAdr = curAdr;
                    throw Exception("Bye!");
        ...
        if (SameText(_name, "@DispInvoke")) {
            ...
            _value = ManualInput(CurProcAdr, curAdr,
                                 "Input return bytes number (in hex) of procedure at " + Val2Str8(curAdr), "Bytes:");
            if (_value == "") {
                Env->ErrAdr = curAdr;
                throw Exception("Bye!");
            }
        ...
        if (!recM) {
            ...
            if (_recN->procInfo->flags & PF_EMBED) {
                ...
                if (Env->EmbeddedList->IndexOf(_embAdr) == -1) {
                    ...
                    if (Application->MessageBox(
                            String("Decompile embedded procedure at address " + _embAdr + "?").c_str(), L"Confirmation",
                            MB_YESNO) == IDYES) {
            ...
            _name = recM->name;
            _res = (int) KnowledgeBase.GetProcInfo(AnsiString(recM->name).c_str(), INFO_DUMP | INFO_ARGS, &_pInfo,
                                                   &_idx);
            if (_res && _res != -1) {
        ...
        if (_argsNum) {
            ...
            if (_callKind == 0 || _callKind == 3 || _callKind == 2 || _callKind == 1) //fastcall, stdcall, pascal, cdecl
            {
                ...
                for (int n = 0; n < _argsNum; n++) {
                    ...
                    if (_kind == ikLString || _kind == ikWString || _kind == ikUString || _kind == ikCString || _kind ==
                        ikWCString) {
                        if (_item.Value != "")
                            _line += _item.Value;
                        else {
    ...
    if (DisInfo.BaseReg != -1) {
        ...
        if (IsValidImageAdr(_classAdr)) {
            ...
            if (GetField(_item.Type, DisInfo.Offset, _name, _type) < 0) {
                while (!_recM) {
                    _typeName = ManualInput(CurProcAdr, curAdr,
                                            "Class (" + _item.Type +
                                            ") has no such virtual method. Give correct class name", "Name:");
                    if (_typeName == "") {
                        Env->ErrAdr = curAdr;
    ...
    if (DisInfo.OpNum == 1 && DisInfo.OpType[0] == otREG) {
        ...
        //_line = _item.Value + ";";
        Env->AddToBody("call...;");
        _value = ManualInput(CurProcAdr, curAdr, "Input stack arguments number of procedure at " + Val2Str8(curAdr),
                             "ArgsNum:");
        if (_value == "") {
            Env->ErrAdr = curAdr;
            throw Exception("Bye!");
...
PLoopInfo __fastcall TDecompiler::GetLoopInfo(int fromAdr) {
    ...
    if (recN && recN->xrefs) {
        ...
        if (_dd1 == 'pmc' && _disInfo.OpType[1] == otIMM) {
            ...
            while (1) {
                ...
                if (_op == OP_MOV || _op == OP_XOR) {
                    if (varIdxInfo.IdxType == itREG && _disInfo.OpType[0] == otREG && IsSameRegister(
                            _disInfo.OpRegIdx[0], varIdxInfo.IdxValue)) {
                    ...
                    if (varIdxInfo.IdxType == itLVAR && _disInfo.OpType[0] == otMEM) {
                        if (_disInfo.BaseReg == 21) //[ebp-N]
                        {
                            ...
                            if (item.IntValue + _disInfo.Offset == varIdxInfo.IdxValue) {
                                ...
                                Env->Stack[varIdxInfo.IdxValue].Value = Env->
                                        GetLvarName(varIdxInfo.IdxValue, "Integer");
                                break;
                            }
                        }
            ...
            while (1) {
                ...
                //mov reg, esp
                if (_op == OP_MOV && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG && _disInfo.OpRegIdx[1]
                    == 20) {
                    idx = _disInfo.OpRegIdx[0];
                    GetRegItem(idx, &item);
                    item.Flags |= IF_ARRAY_PTR;
            ...
            if (!down)
                ...
                to = String(intTo + 1);
            res->forInfo = new TForInfo(noVar, down, stopAdr, from, to, varIdxInfo.IdxType, varIdxInfo.IdxValue, -1,
                                        -1);
            return res;
        }
        if (_dd1 == 'cni' || _dd1 == 'ced') {
                    ...
                    if (_disInfo.OpType[0] == otREG) {
                        ...
                        if (noVar) {
                            ...
                            if (varIdxInfo.IdxType == itREG &&
                                IsSameRegister(_disInfo.OpRegIdx[0], varIdxInfo.IdxValue)) {
                                if (_dd == 'ced') down = true;
                                stopAdr = Pos2Adr(pos);
                            }
                        }
            ...
            res = new TLoopInfo('F', fromAdr, brkAdr, lastAdr); //for
            res->forInfo = new TForInfo(noVar, false, stopAdr, from, to, varIdxInfo.IdxType, varIdxInfo.IdxValue,
                                        cntIdxInfo.IdxType, cntIdxInfo.IdxValue);
            return res;
        }
    }
    res = new TLoopInfo('R', fromAdr, brkAdr, lastAdr); //repeat
            ...
            if (DisInfo.IndxReg == -1) {
                ...
                if (DisInfo.BaseReg == 20) {
                    if (Op == OP_IMUL) {
                        ...
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
                        _line = "EDX_EAX := EAX * " + Env->GetLvarName(_ESP_ + _offset, "Integer") + ";//" + _item1.
                                Value;
                        Env->AddToBody(_line);
                        return;
                    }
                ...
                if (DisInfo.BaseReg == 21 && (_itemBase.Flags & IF_STACK_PTR)) {
                    if (Op == OP_IMUL) {
                        ...
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
                        _line = "EDX_EAX := EAX * " + Env->GetLvarName(_ESP_ + _offset, "Integer") + ";//" + _item.
                                Value;
                        Env->AddToBody(_line);
                        return;
                    }
                ...
                if (_itemBase.Type[1] == '^') //Pointer to gvar (from other unit)
                {
                    if (Op == OP_IMUL) {
                        ...
                        InitItem(&_item);
                        _item.Precedence = PRECEDENCE_MULT;
                        _item.Value = GetString(&_item2, PRECEDENCE_MULT) + " * " + GetString(
                                          &_itemBase, PRECEDENCE_MULT + 1);
                        _item.Type = "Integer";
                        SetRegItem(16, &_item);
                        SetRegItem(18, &_item);
...
void __fastcall TDecompiler::SimulateInstr2RegImm(DWORD curAdr, BYTE Op) {
    ...
    if (Op == OP_SAL || Op == OP_SHL) {
        ...
        if (Op == OP_SHL) {
            ...
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Shl " +
            ...
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " * " +
                    String(_pow2);
            _comment = "";
        }
        SetRegItem(_reg1Idx, &_item);
    ...
    if (Op == OP_SAR || Op == OP_SHR) {
        ...
        if (Op == OP_SHR) {
            ...
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Shr " +
                    String(DisInfo.Immediate);
            _comment = GetString(&_item1, PRECEDENCE_MULT) + " Div " + String(_pow2);
        } else {
            _item.Value = GetString(&_item1, PRECEDENCE_MULT) + " Div " + String(_pow2);
            _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Div " +
                    String(_pow2);
            _comment = "";
        }
        SetRegItem(_reg1Idx, &_item);
...
void __fastcall TDecompiler::SimulateInstr2RegReg(DWORD curAdr, BYTE Op) {
    ...
    if (Op == OP_ADD || Op == OP_SUB || Op == OP_OR || Op == OP_XOR) {
        ...
        _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + _op +
                GetDecompilerRegisterName(_reg2Idx);
...
void __fastcall TDecompiler::SimulateInstr2RegMem(DWORD curAdr, BYTE Op) {
    ...
    if (_itemSrc.Flags & IF_INTVAL) {
        ...
        if (Op == OP_MOV || Op == OP_ADD || Op == OP_SUB || Op == OP_MUL || Op == OP_IMUL || Op == OP_DIV || Op ==
            OP_IDIV) {
            _name = "";
            _type = "";
            _ap = Adr2Pos(_offset);
    ...
    if (Op == OP_ADD || Op == OP_SUB || Op == OP_XOR) {
        ...
        SetRegItem(_reg1Idx, &_item);
        _line = GetDecompilerRegisterName(_reg1Idx) + " := " + GetDecompilerRegisterName(_reg1Idx) + _op + _itemSrc.
                Value + ";//" + _item.Value;
        Env->AddToBody(_line);
        return;
    }
    ...
    if (Op == OP_DIV || Op == OP_IDIV) {
        ...
        _line = GetDecompilerRegisterName(16) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Div " + _itemSrc.Value
                + ";//" + _itemDst.Value + " Div " + _itemSrc.Value;
        Env->AddToBody(_line);
        _line = GetDecompilerRegisterName(18) + " := " + GetDecompilerRegisterName(_reg1Idx) + " Mod " + _itemSrc.Value
                + ";//" + _itemDst.Value + " Div " + _itemSrc.Value;
        Env->AddToBody(_line);
        return;
    }
...
String __fastcall TDecompiler::GetSysCallAlias(String AName) {
    ...
    if (SameText(AName, "@Copy") ||
        ...
        SameText(AName, "@UStrCopy"))
        return "Copy";
    if (SameText(AName, "@LStrDelete") ||
        SameText(AName, "@UStrDelete"))
        return "Delete";
...
bool __fastcall TDecompiler::SimulateSysCall(String name, DWORD procAdr, int instrLen) {
    ...
    if (SameText(name, "@DynArrayAsg")) {
        ...
        GetRegItem(16, &_item1);
        if (_item1.Flags & IF_STACK_PTR) {
            if (Env->Stack[_item1.IntValue].Value == "") Env->Stack[_item1.IntValue].Value = Env->GetLvarName(
                                                             _item1.IntValue, "array of");
            _item1 = Env->Stack[_item1.IntValue];
        }
        //edx - src
        GetRegItem(18, &_item2);
        if (_item2.Flags & IF_STACK_PTR) {
            if (Env->Stack[_item2.IntValue].Value == "") Env->Stack[_item2.IntValue].Value = Env->GetLvarName(
                                                             _item2.IntValue, "array of");
    ...
    if (SameText(name, "@Str2Ext")) {
        ...
        _line = "Str(" + _item1.Value + ":" + String(_item2.IntValue) + ":" + String(_item3.IntValue) + ", " + _item4.
                Value + ");";
        Env->AddToBody(_line);
        return false;
    }
    if (SameText(name, "@Write2Ext")) {
        ...
        //ecx
        GetRegItem(17, &_item4);
        _line = "Write(" + ExtractClassName(_item1.Value) + ", " + _item2.Value + ":" + String(_item3.IntValue) + ":" +
                String(_item4.IntValue) + ");";
        Env->AddToBody(_line);
        return false;
    }
...
void __fastcall TDecompiler::SimulateFloatInstruction(DWORD curAdr) {
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "add")) != 0) {
        ...
        if (DisInfo.OpNum == 2) {
            ...
            InitItem(&_item);
            _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_ADD) + " + " + GetString(
                              FGet(_reg2Idx), PRECEDENCE_ADD + 1);
            _item.Precedence = PRECEDENCE_ADD;
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
            //faddp
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "sub")) != 0) {
        ...
        if (DisInfo.OpNum == 2) {
            ...
            InitItem(&_item);
            if (_r)
                _item.Value = GetString(FGet(_reg2Idx), PRECEDENCE_ADD) + " - " + GetString(
                                  FGet(_reg1Idx), PRECEDENCE_ADD + 1);
            else
                _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_ADD) + " - " + GetString(
                                  FGet(_reg2Idx), PRECEDENCE_ADD + 1);
            _item.Precedence = PRECEDENCE_ADD;
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "mul")) != 0) {
        ...
        if (DisInfo.OpNum == 2) {
            ...
            _reg2Idx = DisInfo.OpRegIdx[1] - 30;
            InitItem(&_item);
            _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_MULT) + " * " + GetString(
                              FGet(_reg2Idx), PRECEDENCE_MULT + 1);
            _item.Precedence = PRECEDENCE_MULT;
            _item.Type = "Extended";
            FSet(_reg1Idx, &_item);
            //fmulp
    ...
    if ((_pos = strstr(DisInfo.Mnem + 1, "div")) != 0) {
        ...
        if (DisInfo.OpNum == 2) {
            ...
            InitItem(&_item);
            if (_r)
                _item.Value = GetString(FGet(_reg2Idx), PRECEDENCE_MULT) + " / " + GetString(
                                  FGet(_reg1Idx), PRECEDENCE_MULT + 1);
            else
                _item.Value = GetString(FGet(_reg1Idx), PRECEDENCE_MULT) + " / " + GetString(
                                  FGet(_reg2Idx), PRECEDENCE_MULT + 1);
            _item.Precedence = PRECEDENCE_MULT;
    ...
    if (DisInfo.IndxReg == -1) {
        ...
        if (_kind == ikLString || _kind == ikString) {
            ...
            if (GetTypeKind(_typeName, &_size) == ikRecord) {
                ...
                _text = GetRecordFields(_offset, _typeName);
                if (_text == "") {
                    _text = ManualInput(CurProcAdr, CurAdr,
                                        "Define [name:]type of field " + _typeName + ".f" + Val2Str0(_offset),
                                        "[Name]:Type:");
                    if (_text == "") {
                        Env->ErrAdr = CurAdr;
                        throw Exception("Bye!");
            ...
            if (_fofs < 0) {
                _text = ManualInput(CurProcAdr, CurAdr,
                                    "Define correct type of field " + _typeName + ".f" + Val2Str0(_offset), "Type:");
                if (_text == "") {
                    Env->ErrAdr = CurAdr;
                    throw Exception("Bye!");
                }
            ...
            if (_kind == ikRecord) {
                ...
                if (_text == "") {
                    _text = ManualInput(CurProcAdr, CurAdr,
                                        "Define [name:]type of field " + _typeName + ".f" + Val2Str0(_offset - _fofs),
                                        "[Name]:Type:");
                    if (_text == "") {
                        Env->ErrAdr = CurAdr;
                        throw Exception("Bye!");
