    ...
    for (pos = fromPos - 1; pos >= toPos; pos--) {
        if (IsFlagSet(cfInstruction, pos)) {
            ...
            if ((len1 && !memicmp(DisInfo.Mnem, Instruction1, len1)) ||
                (len2 && !memicmp(DisInfo.Mnem, Instruction2, len2)))
                return pos;
        }
        if (IsFlagSet(cfProcStart, pos)) break;
    }
...
int __fastcall BranchGetPrevInstructionType(DWord fromAdr, DWord *jmpAdr, PLoopInfo loopInfo) {
    ...
    *jmpAdr = 0;
    int _pos = GetNearestUpInstruction(Adr2Pos(fromAdr));
    if (_pos == -1) return -1;
    Disasm.Disassemble(Pos2Adr(_pos), &_disInfo, 0);
    if (_disInfo.Branch) {
...
int __fastcall GetRecordField(String ARecType, int AOfs, String &name, String &type) {
    ...
    if (_recT && _recT->kind == ikRecord) {
        ...
        _pos += 4;
        for (n = 0; n < _elNum; n++) {
            _typeAdr = *reinterpret_cast<DWord *>(Code + _pos);
            _pos += 4;
            Ofs1 = *reinterpret_cast<DWord *>(Code + _pos);
            _pos += 4;
            if (n == _elNum - 1)
                Ofs2 = 0;
            else
                Ofs2 = *reinterpret_cast<DWord *>(Code + _pos + 4);
            if (AOfs >= Ofs1 && AOfs < Ofs2) {
...
int __fastcall IsTryBegin(DWord fromAdr, DWord *endAdr) {
    ...
    for (n = 1; n <= 5; n++) {
        ...
        _op = Disasm.GetOp(_disInfo.Mnem);
        if (n == 1) {
            if (!(_op == OP_XOR && _disInfo.OpType[0] == _disInfo.OpType[1] && _disInfo.OpRegIdx[0] == _disInfo.OpRegIdx
                  [1])) break;
        } else if (n == 2) {
...
int __fastcall ProcessInt64ComparisonViaStack1(DWord fromAdr, DWord *maxAdr) {
    ...
    for (n = 1; n <= 1024; n++) {
        ...
        } else if (n == 2) //push reg
        {
            if (!(_op == OP_PUSH && _disInfo.OpType[0] == otREG)) break;
            _pushAdr = _curAdr;
        } else if (n >= 3 && m == -1 && _op == OP_CMP && _disInfo.OpType[1] == otMEM && _disInfo.BaseReg == 20 &&
                   _disInfo.Offset == 4) //cmp XXX,[esp+4]
        {
...
int __fastcall IsInt64Shr(DWord fromAdr) {
    ...
    for (n = 1; n <= 2; n++) {
        ...
        _op = Disasm.GetOp(_disInfo.Mnem);
        if (n == 1) {
            if (!(_op == OP_SHR && _disInfo.OpNum == 3 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG &&
                  _disInfo.OpType[2] == otIMM)) break;
            _idx = _disInfo.OpRegIdx[1];
            _val = _disInfo.Immediate;
        } else if (n == 2) {
            if (!(_op == OP_SHR && _disInfo.OpNum == 2 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otIMM &&
                  _disInfo.OpRegIdx[0] == _idx && _disInfo.Immediate == _val)) break;
            return _curAdr + _instrLen - fromAdr;
        }
        if (_disInfo.Ret) return 0;
        _curAdr += _instrLen;
...
int __fastcall IsInt64Shl(DWord fromAdr) {
    ...
    for (n = 1; n <= 2; n++) {
        ...
        _op = Disasm.GetOp(_disInfo.Mnem);
        if (n == 1) {
            if (!(_op == OP_SHL && _disInfo.OpNum == 3 && _disInfo.OpType[0] == otREG && _disInfo.OpType[1] == otREG &&
                  _disInfo.OpType[2] == otIMM)) break;
            _idx = _disInfo.OpRegIdx[1];
            _val = _disInfo.Immediate;
        } else if (n == 2) {
            if (!(_op == OP_SHL && _disInfo.OpNum == 2 && _disInfo.OpType[0] == otREG && _disInfo.OpType[2] == otIMM &&
                  _disInfo.OpRegIdx[0] == _idx && _disInfo.Immediate == _val)) break;
            return _curAdr + _instrLen - fromAdr;
        }
        if (_disInfo.Ret) return 0;
