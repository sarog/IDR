...
void __fastcall TFMain_11011981::AnalyzeProc1(DWORD fromAdr, char xrefType, DWORD xrefAdr, int xrefOfs, bool maybeEmb) {
    ...
    while (1) {
                        ...
                        if (Code[Pos] == 0xE9) //jmp Handle...
                        {
                            ...
                            if (recN1 && recN1->HasName()) {
                                ...
                                if (recN1->SameName("@HandleFinally")) {
                                    ...
                                    if (Code[Pos] == 0x68)
                                    //push offset @3       //Flags[Pos] & cfInstruction must be != 0
                                    {
                                        hfStartPos = Pos - 8;
                                    ...
                                    if (Disasm.GetOp(DisInfo.Mnem) != OP_XOR || DisInfo.OpRegIdx[0] != DisInfo.OpRegIdx[
                                            1]) {
                                        hoStartPos = Pos - 13;
        ...
        if (b1 == 0xEB || //short relative abs jmp or cond jmp
            ...
            if (IsValidCodeAdr(Adr)) {
                Pos = Adr2Pos(Adr);
                if (!IsFlagSet(cfEmbedded, Pos))
                //Possible branch to start of Embedded proc (for ex. in proc TextToFloat))
                {
                    SetFlag(cfLoc, Pos);
        ...
        if (DisInfo.OpType[1] == otIMM) {
            ...
            if (Pos >= 0 && IsValidCodeAdr(DisInfo.Immediate) && (
                    DisInfo.Immediate < fromAdr || DisInfo.Immediate >= fromAdr + procSize)) {
