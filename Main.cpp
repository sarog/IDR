                                    ...
                                    if (!KnowledgeBase.IsUsedProc(Idx)) {
                                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo)) {
                                            if (!SameText(fName, "@Halt")) {
                                                ...
                                                while (_bytes > 0) {
                                                    int _instrlen = Disasm.Disassemble(
                                                        Code + Adr2Pos(Adr), (__int64) Adr, &_disInfo, 0);
                                                    if (_disInfo.Branch && !_disInfo.Conditional) {
                                                        ...
                                                        if (Idx != -1) {
                                                            Idx = KnowledgeBase.ProcOffsets[Idx].NamId;
                                                            if (!KnowledgeBase.IsUsedProc(Idx)) {
                                                                if (KnowledgeBase.GetProcInfo(
                                                                    Idx, INFO_DUMP | INFO_ARGS, pInfo))
                                                                    StrapProc(Adr2Pos(Adr), Idx, pInfo, false,
                                                                              EstimateProcSize(Adr));
                                                            }
                                                        }
                                                        break;
                                                    }
...
int __fastcall TFMain_11011981::GetDelphiVersion() {
    ...
    */
    ...
    const DelphiBplVersionRec delphiBplVer[] = {
        //Borland
    ...
    if (rtlBpl != "" || vclBpl != "") {
        for (int i = 0; delphiBplVer[i].versionForIdr; ++i) {
            ...
            if (!((rtlFile == "" || IsBplByExport(AnsiString(rtlFile).c_str())) && IsBplByExport(
                      AnsiString(vclFile).c_str()))) {
...
bool __fastcall TFMain_11011981::InOneUnit(DWORD Adr1, DWORD Adr2) {
    for (int n = 0; n < UnitsNum; n++) {
        ...
        if (Adr1 >= recU->fromAdr && Adr1 < recU->toAdr &&
            Adr2 >= recU->fromAdr && Adr2 < recU->toAdr)
            return true;
    }
    return false;
}
    ...
    for (row = 0; row < outRows; row++) {
        ...
        if (op == OP_JMP) {
            ...
            if (DisInfo.OpType[0] == otIMM) {
                ...
                if (GetSegmentNo(Adr) != 0 && GetSegmentNo(fromAdr) != GetSegmentNo(Adr) && (
                        !lastAdr || curAdr == lastAdr)) {
                    curAdr += instrLen;
                    break;
                }
                            ...
                            for (int j = 0; j < num1; j++) {
                                ...
                                if (msg1 == msg) {
                                    ...
                                    if (recN2 && recN2->HasName()) {
                                        ...
                                        if (dpos)
                                            recM.name = className + recN2->GetName().SubString(
                                                            dpos, recN2->GetNameLength() - dpos + 1);
                                        else
    ...
    if (!adr1 || !adr2) {
        ...
        if (DelphiVersion >= 2009) {
            ...
            if ((SameText(Name1, "UString") && SameText(Name2, "UnicodeString")) ||
                (SameText(Name1, "UnicodeString") && SameText(Name2, "UString")))
                return "UnicodeString";
        }
        //String - AnsiString
        if ((SameText(Name1, "String") && SameText(Name2, "AnsiString")) ||
            (SameText(Name1, "AnsiString") && SameText(Name2, "String")))
            return "AnsiString";
        //Text - TTextRec
        if ((SameText(Name1, "Text") && SameText(Name2, "TTextRec")) ||
            (SameText(Name1, "TTextRec") && SameText(Name2, "Text")))
            return "TTextRec";
        return "";
    }
...
void __fastcall TFMain_11011981::AnalyzeMethodTable(int Pass, DWORD Adr, const volatile bool *Terminated) {
    ...
    if (DelphiVersion >= 2010) {
        ...
        for (int n = 0; n < exCount && !*Terminated; n++) {
            ...
            if (Pass == 1) {
                if (recN && recN->procInfo && recN->kind != ikConstructor && recN->kind != ikDestructor)
                //recN->kind != ikClassRef
                {
                    recN->SetName(className + "." + methodName);
                    recN->kind = ikProc;
                    recN->AddXref('D', Adr, 0);
    ...
    while (1) {
        ...
        if (op == OP_JMP) {
            ...
            if (DisInfo.OpType[0] == otIMM) {
                ...
                if (GetSegmentNo(Adr) != 0 && GetSegmentNo(fromAdr) != GetSegmentNo(Adr) && (
                        !lastAdr || curAdr == lastAdr)) return 0;
            ...
            while (n >= 0) {
                ...
                if (intfsNum) {
                    for (m = 0; m < intfsNum; m++) {
                        ...
                        if (IsValidCodeAdr(vAdr)) {
                            int pos = item.Pos(' ');
                            TTreeNode *intfsNode = tvClassesShort->Items->AddChild(
                                node, "<I> " + item.SubString(pos + 1, item.Length()));
                            cnt = 0;
                            pos = Adr2Pos(vAdr);
                            for (int v = 0;; v += 4) {
...
void __fastcall TFMain_11011981::IniFileWrite() {
    ...
    for (n = 0; n < 8; n++) {
        ...
        if (item->Visible && item->Enabled) iniFile->WriteString("Recent Executable Files", "File" + String(n + 1),
                                                                 "\"" + item->Caption + "\"," + String(item->Tag));
    }
    for (n = 9; n < 17; n++) {
        TMenuItem *item = miMRF->Items[n];
        if (item->Visible && item->Enabled) iniFile->WriteString("Recent Project Files", "File" + String(n - 8),
                                                                 "\"" + item->Caption + "\"");
    }
...
int __fastcall TFMain_11011981::LoadImageFile(FILE *f, int version, bool loadExp, bool loadImp) {
    ...
    //temporary assignment to evaluate BCB ini and fin tables
    BCB = false;
    ...
    if (version != 2) {
        ...
        if (evalEP + CodeBase != NTHeaders.OptionalHeader.AddressOfEntryPoint + ImageBase) {
            sprintf(msg, "Possible invalid EP (NTHeader:%lX, Evaluated:%lX). Input valid EP?",
                    NTHeaders.OptionalHeader.AddressOfEntryPoint + ImageBase, evalEP + CodeBase);
            if (Application->MessageBox(String(msg).c_str(), L"Confirmation", MB_YESNO) == IDYES) {
                sEP = InputDialogExec("New EP", "EP:",
                                      Val2Str0(NTHeaders.OptionalHeader.AddressOfEntryPoint + ImageBase));
                if (sEP != "") {
...
void __fastcall TFMain_11011981::OpenProject(String FileName) {
    ...
    if (fIn) {
        ...
        for (n = 0; n < num; n++) {
            PSegmentInfo segInfo = new SegmentInfo;
            fread(&segInfo->Start, sizeof(segInfo->Start), 1, fIn);
            //inStream->Read(&segInfo->Start, sizeof(segInfo->Start));
            fread(&segInfo->Size, sizeof(segInfo->Size), 1, fIn);
            //inStream->Read(&segInfo->Size, sizeof(segInfo->Size));
            fread(&segInfo->Flags, sizeof(segInfo->Flags), 1, fIn);
        ...
        for (n = 0; n < UnitsNum; n++) {
            ...
            fread(&recU->trivial, sizeof(recU->trivial), 1, fIn);
            //inStream->Read(&recU->trivial, sizeof(recU->trivial));
            fread(&recU->trivialIni, sizeof(recU->trivialIni), 1, fIn);
            //inStream->Read(&recU->trivialIni, sizeof(recU->trivialIni));
            fread(&recU->trivialFin, sizeof(recU->trivialFin), 1, fIn);
            ...
            fread(&recU->fromAdr, sizeof(recU->fromAdr), 1, fIn);
            ...
            fread(&recU->finSize, sizeof(recU->finSize), 1, fIn);
            ...
            fread(&recU->iniSize, sizeof(recU->iniSize), 1, fIn);
            //inStream->Read(&recU->iniSize, sizeof(recU->iniSize));
            recU->matchedPercent = 0.0;
            fread(&recU->iniOrder, sizeof(recU->iniOrder), 1, fIn);
        ...
        if (UnitsNum) {
            fread(&UnitSortField, sizeof(UnitSortField), 1, fIn);
        ...
        fread(&CodeHistoryMax, sizeof(CodeHistoryMax), 1, fIn);
        ...
        for (n = 0; n < CodeHistorySize; n++)
            fread(&CodeHistory[n], sizeof(PROCHISTORYREC), 1, fIn);
    ...
    try {
        ...
        if (outF) {
            ...
            for (n = 0; n < num; n++) {
                ...
                fwrite(&segInfo->Flags, sizeof(segInfo->Flags), 1, outF);
            ...
            for (n = 0; n < num; n++) {
                ...
                fwrite(&recU->trivialFin, sizeof(recU->trivialFin), 1, outF);
                ...
                fwrite(&recU->fromAdr, sizeof(recU->fromAdr), 1, outF);
                //outStream->Write(&recU->fromAdr, sizeof(recU->fromAdr));
                fwrite(&recU->toAdr, sizeof(recU->toAdr), 1, outF);
                //outStream->Write(&recU->toAdr, sizeof(recU->toAdr));
                fwrite(&recU->finadr, sizeof(recU->finadr), 1, outF);
                //outStream->Write(&recU->finadr, sizeof(recU->finadr));
                fwrite(&recU->finSize, sizeof(recU->finSize), 1, outF);
                //outStream->Write(&recU->finSize, sizeof(recU->finSize));
                fwrite(&recU->iniadr, sizeof(recU->iniadr), 1, outF);
                //outStream->Write(&recU->iniadr, sizeof(recU->iniadr));
                fwrite(&recU->iniSize, sizeof(recU->iniSize), 1, outF);
                //outStream->Write(&recU->iniSize, sizeof(recU->iniSize));
                fwrite(&recU->iniOrder, sizeof(recU->iniOrder), 1, outF);
            ...
            if (num) {
                fwrite(&UnitSortField, sizeof(UnitSortField), 1, outF);
            ...
            fwrite(&CodeHistoryMax, sizeof(CodeHistoryMax), 1, outF);
            //outStream->Write(&CodeHistoryMax, sizeof(CodeHistoryMax));
            PROCHISTORYREC phRec;
            FProgressBar->StartProgress("Writing Code History Items (number = " + String(CodeHistorySize) + ")...", "",
                                        CodeHistorySize);
            for (n = 0; n < CodeHistorySize; n++) {
                FProgressBar->pb->StepIt();
                Application->ProcessMessages();
                fwrite(&CodeHistory[n], sizeof(PROCHISTORYREC), 1, outF);
            ...
            fwrite(&LastResStrNo, sizeof(LastResStrNo), 1, outF);
            ...
            if (num && Application->MessageBox(L"Save full Tree of Classes?", L"Warning", MB_YESNO) == IDYES) {
                ...
                fwrite(&num, sizeof(num), 1, outF); //outStream->Write(&num, sizeof(num));
                if (num) {
                    FProgressBar->StartProgress("Writing ClassViewer Tree Nodes (number = " + String(num) + ")...", "",
                                                num);
...
void __fastcall TFMain_11011981::wm_dropFiles(TWMDropFiles &msg) {
    ...
    try {
        for (int i = 0; i < fc->FileCount; ++i) {
            ...
            else if ((SameText(ext, ".exe") || SameText(ext, ".bpl") || SameText(ext, ".dll") || SameText(ext, ".scr"))
                     && miLoadFile->Enabled)
...
void __fastcall TFMain_11011981::FormCloseQuery(TObject *Sender, bool &CanClose) {
    ...
    if (AnalyzeThread) {
        ...
        _res = Application->MessageBox(L"Analysis is not yet completed. Do You really want to exit IDR?",
                                       L"Confirmation", MB_YESNO);
        if (_res == IDNO) {
...
void __fastcall TFMain_11011981::FillClassViewerOne(int n, TStringList *tmpList, const volatile bool *Terminated) {
    ...
    if (DelphiVersion >= 2009) sizeParent += 4;
    ...
    if (rootNode) {
        ...
        if (intfsNum) {
            for (m = 0; m < intfsNum && !*Terminated; m++) {
                ...
                if (IsValidCodeAdr(vAdr)) {
                    TTreeNode *intfsNode = AddClassTreeNode(
                        rootNode, "<I> " + nodeText.SubString(nodeText.Pos(' ') + 1, nodeText.Length()));
                    cnt = 0;
                    pos = Adr2Pos(vAdr);
                    for (int v = 0;; v += 4) {
                        if (IsFlagSet(cfVTable, pos)) cnt++;
