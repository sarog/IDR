                        ...
                        if (KnowledgeBase.GetTypeInfo(TypeIdx, INFO_FIELDS, tInfo)) {
                            if (tInfo->Fields) {
                                ...
                                for (int n = 0; n < tInfo->FieldsNum; n++) {
                                    ...
                                    p += Len + 1;
                                    recN->vmtInfo->AddField(0, 0, fInfo.Scope, fInfo.Offset, fInfo.Case, fInfo.Name,
                                                            fInfo.Type);
                                }
                            }
                        }
...
typedef struct {
    bool used;
    const char *name;    // имя юнита | unit name
    float matched; // максимальное кол-во совпадений | maximum number of matches
    int maxno;     // номер юнита с максимальным кол-вом совпадений | unit number with the maximum number of matches
                ...
                if (Idx != -1) {
                    ...
                    if (!KnowledgeBase.IsUsedProc(Idx)) {
                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo)) {
                            matched = (MatchCode(Code + Adr2Pos(iniAdr), pInfo) && mainForm->StrapCheck(
                                           Adr2Pos(iniAdr), pInfo));
        ...
        if (finAdr && !recU->trivialFin) {
            ...
            for (u = 0; u < recU->names->Count && !Terminated; u++) {
                ...
                if (Idx != -1) {
                    ...
                    if (!KnowledgeBase.IsUsedProc(Idx)) {
                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo)) {
                            matched = (MatchCode(Code + Adr2Pos(finAdr), pInfo) && mainForm->StrapCheck(
                                           Adr2Pos(finAdr), pInfo));
                ...
                for (m = fromPos; m < toPos && !Terminated; m++) {
                    if (IsFlagSet(cfProcStart, m)) {
                        ...
                        for (k = FirstProcIdx; k <= LastProcIdx && !Terminated; k++) {
                            ...
                            if (!KnowledgeBase.IsUsedProc(Idx)) {
                                matched = false;
                                if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP, pInfo) && pInfo->DumpSz >= 8 && m + pInfo
                                    ->DumpSz < toPos) {
            ...
            for (m = fromPos; m < toPos && !Terminated; m++) {
                if (IsFlagSet(cfProcStart, m)) {
                    ...
                    for (k = FirstProcIdx; k <= LastProcIdx && !Terminated; k++) {
                        ...
                        if (!KnowledgeBase.IsUsedProc(Idx)) {
                            matched = false;
                            if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo) && pInfo->DumpSz >= 8 && m
                                + pInfo->DumpSz < toPos) {
                                matched = (MatchCode(Code + m, pInfo) && mainForm->StrapCheck(m, pInfo));
                                if (matched) {
                                    //If method of class, check that ClassName is found
...
void __fastcall TAnalyzeThread::ScanConsts() {
    ...
    HINSTANCE hInst = LoadLibraryEx(AnsiString(mainForm->SourceFile).c_str(), 0, LOAD_LIBRARY_AS_DATAFILE);
    ...
    for (int m = 0; m < OwnTypeList->Count && !Terminated; m++) {
        ...
        if (recT->kind == ikClass) {
            ...
            for (int i = 0; i < propCount && !Terminated; i++) {
                ...
                if (recN1 && fieldOfs != -1) recN1->vmtInfo->AddField(0, 0, FIELD_PUBLIC, fieldOfs, -1, fieldName,
                                                                      fieldType);
            }
        }
    }
