...
void __fastcall TFKBViewer_11011981::btnOkClick(TObject *Sender) {
    ...
    if (KnowledgeBase.GetProcInfo(CurrIdx, INFO_DUMP | INFO_ARGS, pInfo)) {
                        ...
                        if (kbName != "" && ap >= 0) {
                            ...
                            if (Idx != -1) {
                                ...
                                if (Idx != -1) {
                                    ...
                                    if (!KnowledgeBase.IsUsedProc(Idx)) {
                                        if (KnowledgeBase.GetProcInfo(Idx, INFO_DUMP | INFO_ARGS, pInfo))
                                            FMain_11011981->StrapProc(ap, Idx, pInfo, false,
                                                                      FMain_11011981->EstimateProcSize(adr));
                                    }
                                }
                            }
                        }
        ...
        FMain_11011981->ShowUnitItems(FMain_11011981->GetUnit(CurUnitAdr), FMain_11011981->lbUnitItems->TopIndex,
                                      FMain_11011981->lbUnitItems->ItemIndex);
    }
    Close();
}
